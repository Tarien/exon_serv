local config = {
	...
	lockStorage = 60602, -- globalstorage
	bossPos = Position(33223, 33100, 9),
	centerRoom = Position(33223, 33103, 9), -- Center Room  
	exitPosition = Position(33199, 33128, 9), -- Exit Position
	newPos = Position(33223, 33110, 9),
	range = 10,
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 1945 and item.actionid == 60602 then
		if player:getPosition() ~= Position(33199, 33122, 9) then
			return true
		end
			
	for y = 33122, 33126 do
	local playerTile = Tile(Position(33199, y, 9)):getTopCreature()
		if playerTile and playerTile:isPlayer() then 
			-- if playerTile:getStorageValue(Storage.Aurelius.LeverTimer) > os.time() then
	...
	for i = 1, #specs do
		...
		if spec:isPlayer() then
			player:sendTextMessage(MESSAGE_STATUS_SMALL, "There's someone fighting with Tempestbossplaceholder.")
			item:transform(1946)
			return true
		end
	end	
	...
	for y = 33122, 33126 do
		...
		if playerTile and playerTile:isPlayer() then 					
			...
			playerTile:setStorageValue(Storage.Aurelius.LeverTimer, os.time() + 20 * 60 * 60) -- + 20 * 60 * 3600
